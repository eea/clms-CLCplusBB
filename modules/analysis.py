...
def get_boundary_length_per_class_pair(
    ...
    """
    Calculate pairwise boundary lengths (km), edge densities (m/ha),
    and relative fractions of total boundary length / edge density
    ...
    if len(matches) > 1:
        print("Warning: multiple rasters matched; using first:")
        for m in matches:
            print("  ", m)
    ...
    if np.any(right_mask):
        ...
        pairs_right = np.stack(
            [right_a[right_mask], right_b[right_mask]],
            axis=1
        )
    ...
    if np.any(bottom_mask):
        ...
        pairs_bottom = np.stack(
            [bottom_a[bottom_mask], bottom_b[bottom_mask]],
            axis=1
        )
    ...
    if class_info is not None:
        ...
        for (class_a, class_b), length in boundary_lengths.items():
            ...
            name_a = class_info.get(class_a, {}).get(
                'name', f'Class {class_a}'
            )

            name_b = class_info.get(class_b, {}).get(
    ...
    # -------------------------
    # totals for relative metrics
    # -------------------------

    total_edge_length = sum(boundary_lengths.values())

    total_edge_density = (
        total_edge_length / (total_area / 10000.0)
    ...
    for key, length_m in boundary_lengths.items():
        ...
        edge_density_m_per_ha = (
            length_m / (total_area / 10000.0)
            if total_area > 0 else 0.0
        )

        rel_length = (
            length_m / total_edge_length
            if total_edge_length > 0 else 0.0
        )

        rel_edge_density = (
            edge_density_m_per_ha / total_edge_density
            if total_edge_density > 0 else 0.0
        )

        entry = {
            'length_km': round(length_km, 3),
            'rel_length_fraction': round(rel_length, 4)
        }

        if return_density:

            entry['edge_density_m_per_ha'] = round(
                edge_density_m_per_ha, 3
            )

            entry['rel_edge_density_fraction'] = round(
                rel_edge_density, 4
            )
