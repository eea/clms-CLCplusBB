...
def prepare_layer_to_map_discrete(
    rasters_dir,
    chosen_region,
    layer_dict,
    class_info,
    ...
    """
    Process a discrete/categorical raster dataset and export a PNG
    at the raster's native resolution for clean Folium rendering.
    """
    ...
    for val in unique_vals:
        if not np.isnan(val):
            ...
            colors.append(class_data.get('color', '#808080'))
            class_names.append(class_data.get('name', f'Class {val_int}'))
    ...
    # map raster values to colormap indices
    ...
    # mask nodata
    arr_indexed = np.ma.masked_where(
        (arr_indexed == -1) | (arr == 0),
        arr_indexed
    )

    height, width = arr_indexed.shape
    ...
    ax.imshow(
        ...
        cmap=cmap,
        norm=norm,
        interpolation='nearest'  # important for categorical data
    )
...
def display_map_discrete(
    ...
    """
    Create a Folium map displaying a discrete/categorical raster layer
    ...
    m = folium.Map(
        coordinates,
        zoom_start=regions_dict[chosen_region][1],
        tiles=base_map
    ).add_to(figure)
    ...
    for color, name in zip(colors, class_names):
        ...
        <i style="background:{color}; width:15px; height:15px; 
                  float:left; margin-right:8px; margin-top:2px;"></i>
